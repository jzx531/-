...
gray_img = cv2.cvtColor(img5, cv2.COLOR_RGB2GRAY)
cv2.imshow("resized_5",gray_img)
gray_img=cv2.resize(gray_img,(28,28),interpolation= cv2.INTER_LINEAR)
#print(gray_img.shape)
gray_img_tensor=torch.tensor(gray_img,dtype=torch.float)
gray_img_tensor=torch.unsqueeze(gray_img_tensor,dim=0)
gray_img_tensor=torch.unsqueeze(gray_img_tensor,dim=0)
#print(gray_img_tensor.size())
print(f"预测为{torch.max(cnn(gray_img_tensor), 1)[1].data.numpy()}")
cv2.waitKey()
